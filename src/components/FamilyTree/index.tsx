...
export function FamilyTree({ data, isDark }: Props) {
  ...
  const { width: svgW, height: svgH } = calcSvgDimensions(
    totalWidth,
    totalHeight
  );
  const PADDING = 60;
        ...
        >
          ...
          >
            <g transform={`translate(${PADDING}, ${PADDING})`}>
              ...
              {partnerEdges.map(
                (pe: { x1: number; x2: number; y: number }, i: number) => (
                  <PartnerLine
                    key={i}
                    x1={pe.x1}
                    x2={pe.x2}
          ...
          <div
            className='absolute'
            style={{ left: PADDING, top: PADDING, width: svgW, height: svgH }}
          >
            {data.map(member => {
